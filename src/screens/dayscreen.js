...
export default class DaysScreen extends Component {
            ...
            <div>
                ...
                <div className="content">
                    <ul className="table-view" style={{borderBottom: 'none'}}>
                        {this.state.users.map(user => <DriverButton
                            key={user.email}
                        ...
                        <li onClick={this.toggle} style={{borderBottom: 'none'}} className="table-view-cell">
                            {
                                isAlreadyValidated
                                    ? (this.state.changed ?
                                        <p>{`${this.state.driver} will be the driver after update`}</p> :
                                        <p>{`${this.state.driver} is the driver for that day`}</p>)
                            ...
                            <br/>
                            {
                                isAlreadyValidated
                                    ? this.state.changed ?
                                        <button style={{fontSize: 18, fontWeight: 400}} className={buttonClass}
                                                onClick={this.validate}>Update</button> : <span/>
                                    : <button style={{fontSize: 18, fontWeight: 400}} className={buttonClass}
